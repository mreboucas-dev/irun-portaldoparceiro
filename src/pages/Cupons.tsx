  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Acompanhe o desempenho das suas recompensas</p>
      </div>
        ...
        {cuponsData.map((cupom, i) => (
          <GlassCard key={cupom.id} className="animate-fade-in-up" delay={i * 80}>
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="font-semibold text-foreground">{cupom.nome}</h3>
                <p className="text-sm text-muted-foreground">{cupom.categoria}</p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GlassCard className="animate-fade-in-up stagger-4">
          <h3 className="text-lg font-semibold mb-4 text-foreground">CTR por Cupom</h3>
          <ChartContainer config={ctrConfig} className="h-[250px]">
        ...
        <GlassCard className="animate-fade-in-up stagger-5">
          <h3 className="text-lg font-semibold mb-4 text-foreground">Heatmap de Resgates</h3>
          <div className="overflow-x-auto">
            <div className="grid gap-0.5" style={{ gridTemplateColumns: `60px repeat(17, 1fr)` }}>
              <div />
              {Array.from({ length: 17 }, (_, i) => (
                <div key={i} className="text-xs text-center text-muted-foreground">{i + 6}h</div>
              ))}
              {["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"].map((dia) => (
                <>
                  <div key={dia} className="text-xs text-muted-foreground flex items-center">{dia}</div>
                  {heatmapData
                    .filter((h) => h.dia === dia)
                    .map((h, i) => {
